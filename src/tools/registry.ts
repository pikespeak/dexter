...
import { createSportsSearch } from './sports/index.js';
...
import { SPORTS_SEARCH_DESCRIPTION } from './sports/descriptions/index.js';
...
export function getToolRegistry(model: string): RegisteredTool[] {
  ...
  // Include sports_search if API-Football or Odds API key is configured
  if (process.env.API_FOOTBALL_KEY || process.env.ODDS_API_KEY) {
    tools.push({
      name: 'sports_search',
      tool: createSportsSearch(model),
      description: SPORTS_SEARCH_DESCRIPTION,
    });
  }
